...
/// <summary>
/// The tag and repository a run is for. A run already has to start from a checkout
/// of the repository, because the commit range is read with git from there - so
/// that checkout is also where both default from, and the flags only override.
/// A default is announced before the run, because <c>generate</c> publishes to it
/// and nobody should find out which release they wrote to afterwards.
/// </summary>
...
internal sealed partial record ReleaseTarget(string Tag, RepositoryCoordinates Repository)
{
    ...
    /// <summary>
    /// Resolves the target from the arguments, reading from the checkout what they
    /// leave out. Returns <c>null</c> after writing why to <paramref name="error"/>.
    /// </summary>
    ...
    /// and <c>git@host:owner/name</c>, each with or without <c>.git</c>. The host is
    /// not checked, so a GitHub Enterprise remote resolves the same way.
    /// </summary>
